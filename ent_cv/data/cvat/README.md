...
- **combine** — Merge multiple exports into one training dataset. Handles class label remapping across datasets with interactive resolution. Outputs to `datasets/current/`.
...
3. **Combine** — `ent-cv cvat combine` → `datasets/current/`
4. **Split** — `ent-cv prepare-dataset datasets/current/` → adds train/val split
5. **Train** — `ent-cv train --data datasets/current/data_with_val.yaml`
